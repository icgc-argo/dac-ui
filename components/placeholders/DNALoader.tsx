...
const LoaderContainer = styled<'div', { dotsCount: number }>('div')`
  padding-top: 15px;
  padding-bottom: 15px;
  width: ${({ dotsCount }: { dotsCount: number }) => dotsCount * 10}px;
  ...
  @keyframes animateFirstDots {
    0% {
      ...
      background-color: ${({ theme }: { theme: UikitTheme }) => theme.colors.accent1};
    }
    100% {
  ...
  @keyframes animateSecondDots {
    0% {
      transform: translateY(200%) scale(0.7, 0.7);
      background-color: ${({ theme }: { theme: UikitTheme }) => theme.colors.accent4};
    }
    100% {
      transform: translateY(-200%) scale(1, 1);
      background-color: ${({ theme }: { theme: UikitTheme }) => theme.colors.warning};
    }
  }
`;
