...
const ITAgreements = ({
  ...
  return (
    <article>
      ...
      <section>
        <Typography bold component="h3" color="secondary">
          CLOUD STORAGE ACCESS AGREEMENT
        </Typography>
        ...
        <Typography
          ...
          `}
        >
          Optionally, you may apply for and be approved for access to ICGC Controlled Data that is
          pre-loaded into cloud storage and made available for use via ICGC Data Coordinating Centre
          (DCC) authorization tokens. When accessing pre-loaded ICGC Controlled data within a public
          computer cloud service, the DCC will provide you with instruction on locating, accessing
          and decrypting the data.
        </Typography>

        <Typography bold>
          You must agree to the following procedures, although it isn’t necessary that you use the
          cloud.
        </Typography>

        <FormControl
          error={!!localState.agreements?.fields?.it_agreement_cloud_usage_risk?.error}
          required={isRequired(localState.agreements?.fields?.it_agreement_cloud_usage_risk)}
        >
          <FormCheckbox
            aria-label="You understand that your use of the compute-cloud based storage of the ICGC Controlled
            Data is at your own risk."
            disabled={isSectionDisabled}
            checked={localState.agreements?.fields?.it_agreement_cloud_usage_risk?.value}
            onBlur={validateFieldTouched}
            onChange={validateFieldTouched}
            value="agreements--it_agreement_cloud_usage_risk"
          >
            <Typography bold component="span">
              Yes
            </Typography>
            , You understand that your use of the compute-cloud based storage of the ICGC Controlled
            Data is at your own risk.
          </FormCheckbox>

          <FormHelperText onErrorOnly>
            {localState.agreements?.fields?.it_agreement_cloud_usage_risk?.error?.[0]}
          </FormHelperText>
        </FormControl>

        <FormControl
          error={!!localState.agreements?.fields?.it_agreement_read_cloud_appendix?.error}
          required={isRequired(localState.agreements?.fields?.it_agreement_read_cloud_appendix)}
        >
          <FormCheckbox
            aria-label="You have read and understand the guidelines contained in ICGC Best Practices for
            securing Controlled Data in the Cloud."
            disabled={isSectionDisabled}
            checked={localState.agreements?.fields?.it_agreement_read_cloud_appendix?.value}
            onBlur={validateFieldTouched}
            onChange={validateFieldTouched}
            value="agreements--it_agreement_read_cloud_appendix"
          >
            <Typography bold component="span">
              Yes
            </Typography>
            , You have read and understand the guidelines contained in ICGC Best Practices for
            securing Controlled Data in the Cloud (
            <Link>
              Visit APPENDIX VII - ICGC DCC: BEST PRACTICES FOR SECURING CONTROLLED DATA IN THE
              CLOUD (2015)
            </Link>
            ).
          </FormCheckbox>
          ...
          <FormHelperText onErrorOnly>
            {localState.agreements?.fields?.it_agreement_read_cloud_appendix?.error?.[0]}
          </FormHelperText>
        </FormControl>
      </section>
    </article>
