...
export const getStatusText = (state: ApplicationState, dates: StatusDates) => {
  ...
  switch (state) {
    case ApplicationState.APPROVED:
      return `Approved on ${formatStatusDate(dates.approvedAtUtc)}. You now have access to ICGC Controlled Data.`;
    ...
    case ApplicationState.REVIEW:
      return `Submitted on ${formatStatusDate(dates.submittedAtUtc)}. This application is locked for ICGC DACO review.`;
    case ApplicationState.REVISIONS_REQUESTED:
      return `Reopened on ${formatStatusDate(dates.lastUpdatedAtUtc)}. Revision details were sent via email.`;
    case ApplicationState.REJECTED:
      return `Rejected on ${formatStatusDate(dates.lastUpdatedAtUtc)}. This application cannot be reopened, reasons were sent via email.`;
    case ApplicationState.CLOSED:
      return `Closed on ${formatStatusDate(dates.closedAtUtc)}. You can reopen this application at anytime.`;
